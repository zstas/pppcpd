...
bool VPPAPI::add_route( fpm::Message &m ) {
    ...
    if( !add_route.nexthops()[0].has_address() ) {
    ...
    if( add_route.address_family() == qpb::AddressFamily::IPV4 ) {
        ...
        auto &nexthop = add_route.nexthops()[0].address().v4();
        req.route.paths->nh.address.ip4[0] = ( nexthop.value() & 0xFF000000 ) >> 24;
        req.route.paths->nh.address.ip4[1] = ( nexthop.value() & 0x00FF0000 ) >> 16;
        req.route.paths->nh.address.ip4[2] = ( nexthop.value() & 0x0000FF00 ) >> 8;
        req.route.paths->nh.address.ip4[3] = ( nexthop.value() & 0x000000FF );
